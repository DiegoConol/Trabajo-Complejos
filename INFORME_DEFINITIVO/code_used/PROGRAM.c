...
int memoria_usada=0;        //Registro desde el último borrado
int total_vistos_demonio= 0; //Actuación total del demonio
int total_pasan_demonio=0;  //Veces que el demonio deja pasar
...
int main(void)
{
    ...
    //Lo guardará en la carpeta FILES/

    //archivos básicos
    FILE *matriz_file = fopen("FILES/MATRIZ.txt", "w");       //Fichero donde se guarda la matriz y sus pasos. El paso se diferencia por salto de línea vacío.
    FILE *demonio_file = fopen("FILES/demonio.txt", "w");     //Fichero donde se se guarda la partícula sobre la que ha actuado el demonio. 0=no ha actuado, 1=sobre fría, 2=sobre caliente. El paso se diferencia por salto de línea vacío.
    FILE *densidad_file = fopen("FILES/densidad.txt", "w");   //Fichero donde se guarda la densidad de cada sección de la matriz grande en una matriz más pequeña. El paso se diferencia por salto de línea vacío.    
    FILE *presion_cold_file = fopen("FILES/presionL.txt", "w");     //Fichero donde se guarda la presión de las partículas frías. El paso se diferencia por salto de línea vacío.
    FILE *presion_hot_file = fopen("FILES/presionR.txt", "w");
    FILE *memoria_file = fopen("FILES/memoria.txt", "w");
    FILE *hamiltoniano_file = fopen("FILES/hamiltoniano.txt", "w");
    FILE *presion_mitad_file = fopen("FILES/presion_mitad.txt", "w"); //Mide la presión en el muro de enmedio.



    //archivos de entropía
    FILE *densidad_hot_file = fopen("FILES/densidad_hot.txt", "w"); //Densidad de calientes.
    FILE *densidad_cold_file = fopen("FILES/densidad_cold.txt", "w"); //Densidad de frias.
    FILE *entropy_sub_file = fopen("FILES/entropy_subdivisions.txt", "w"); //En este archivo irá la evolución de la entropía por subsecciones.
    FILE *entropy_total_file = fopen("FILES/entropy_total.txt", "w"); //Y aquí la total, que será un número en cada iteración.
    FILE *entropy_demon_file = fopen("FILES/entropy_demon.txt", "w"); //Archivo de entropía y se le suma la entropía del demonio, es decir, su calor.


    //archivos media
    FILE *densidad_file_media = fopen("FILES/densidad_media.txt", "w");   //Fichero donde se guarda la densidad de cada sección de la matriz grande en una matriz más pequeña. El paso se diferencia por salto de línea vacío.    
    FILE *presion_cold_file_media = fopen("FILES/presionL_media.txt", "w");     //Fichero donde se guarda la presión de las partículas frías. El paso se diferencia por salto de línea vacío.
    FILE *presion_hot_file_media = fopen("FILES/presionR_media.txt", "w");
    FILE *entropy_sub_media_file = fopen("FILES/entropy_subdivisions_media.txt", "w"); //En este archivo irá la evolución de la entropía por subsecciones.
    FILE *entropy_total_media_file = fopen("FILES/entropy_total_media.txt", "w"); //Y aquí la total, que será un número en cada iteración.

    // Voy a repetir el proceso bastantes veces para hacer una media estocástica
    FILE *AveragePresion_file = fopen("FILES/Ave_Presion.txt", "w");
    FILE *AverageDensity_file = fopen("FILES/Ave_Density.txt", "w");
    FILE *AverageDensityTypes_file = fopen("FILES/Ave_Density_Types.txt", "w");
    FILE *AverageEntropy_file = fopen("FILES/Ave_Entropy.txt", "w");
    FILE *Average_DemonEntropy_file = fopen("FILES/Ave_Demon_and_Entropy.txt", "w");

    //Para los errores y las sigmas

    FILE *error_presion_file = fopen("FILES/Error_Presion.txt", "w");
    FILE *sigma_presion_file = fopen("FILES/Sigma_Presion.txt", "w");
    FILE *error_densidad_file = fopen("FILES/Error_Densidad.txt", "w");
    FILE *sigma_densidad_file = fopen("FILES/Sigma_Densidad.txt", "w");
    FILE *error_densidad_tipo_file = fopen("FILES/Error_Densidad_Tipo.txt", "w");
    FILE *sigma_densidad_tipo_file = fopen("FILES/Sigma_Densidad_Tipo.txt", "w");
    FILE *error_entropy_file = fopen("FILES/Error_Entropia.txt", "w");
    FILE *sigma_entropy_file = fopen("FILES/Sigma_Entropia.txt", "w");
    FILE *error_entropy_demon_file = fopen("FILES/Error_Entropia_Demonio.txt", "w");
    FILE *sigma_entropy_demon_file = fopen("FILES/Sigma_Entropia_Demonio.txt", "w");
    ...
    for (int r=0; r<repeticiones; r++)
    {
        ...
        memoria_usada=0;
        total_vistos_demonio=0;
        total_pasan_demonio=0;
                                        ...
                                        else //Ya no estamos en la primera fila
                                        {
                                            if(matriz_auxiliar[i-1][j]==0) //Compruebo si está vacía la celda de arriba
                                            {
                                                if ((i)%divisor_fila == 0)
                                                {
                                                    ...
                                                    memoria_usada++;
                                                    total_vistos_demonio++;
                                                    total_pasan_demonio++;

                                                    if(memoria_usada >= MEMORIA)
                                                    {
                                                        Popen=(double) total_pasan_demonio/(double)total_vistos_demonio;
                                                        Pclose=1.0-Popen;
                                                        H=0.0;
                                                        if(Popen > 0.0)
                                                        {
                                                        ...
                                                        if(Pclose > 0.0)
                                                        {
                                                            H +=-Pclose*log2(Pclose);
                                                        }
                                                        entropiaFINALFINAL += MEMORIA*H*log(2.0);
                                                        
                                                        if(vecesborramos < tope)
                                                        {
                                                            H_total[vecesborramos][0] = MEMORIA*H;
