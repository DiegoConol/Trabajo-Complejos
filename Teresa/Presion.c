...
// PRESIÓN #####################################################################################################################
// Hacemos una matriz de un array. La matriz será una matriz con el grid de separaciones, y en cada componente irá un array de 4 elementos
// Primer elemento arriba, derecha, izquierda, abajo. si una partícula se quiere mover en la pared del grid en esa dirección se le hace un +1

int presion[div_N][div_M][4]; 
...
int main(void)
{
    ...
    FILE *presion_file = fopen("PRESION.txt", "w");    //Fichero donde se guarda la matriz presion y sus pasos

    if (presion_file == NULL) {
        printf ("Error al abrir el archivo JAJAJA. \n");
        return 1;
    }
    ...
    {   
        ...
        for (int i=0; i<N; i++)
        {
            for (int j=0; j<M; j++)
            {   
                int fila = i/div_N; //Esto me da la fila del grid en la que estoy. Por ejemplo, si div_N=3, pues las filas 0,1,2 estarán en la fila 0 del grid, las filas 3,4,5 estarán en la fila 1 del grid y las filas 6,7,8 estarán en la fila 2 del grid.
                int columna = j/div_M; //Lo mismo pero para la columna. Si div_M
